...
async function renderKanban() {
  ...
  main.innerHTML = pageHead("Kanban", "PostgreSQL là authority; Drive chỉ là collaboration replica. Di chuyển trạng thái bằng typed transition.", `<button class="button" data-action="kanban-sync">Sync Drive Inbox</button><button class="button button-primary" data-action="new-card-focus">Tạo Card</button>`) + `
    <section class="kanban-layout">
      <div>
        <form id="new-card-form" class="panel inline-form spaced-form">
        ...
        <div class="kanban-board">${CARD_COLUMNS.map(([key,label,tone]) => `<section class="kanban-column"><div class="kanban-head ${tone}"><span>${label}</span><span>${cards.filter((c) => cardColumn(c) === key).length}</span></div>${cards.filter((c) => cardColumn(c) === key).map(renderCardButton).join("") || `<div class="muted compact-note">Trống</div>`}</section>`).join("")}</div>
      </div>
...
function renderCardButton(card) {
  ...
  return `<button type="button" class="card-item${selectedClass}" data-card="${esc(card.card_id)}"><div class="card-id">${esc((card.card_id || "").slice(0,8))}</div><div class="card-title">${esc(card.title || "Không tiêu đề")}</div>${badge(card.status)}<div class="secondary">${esc(card.assignee_agent_id || "Chưa giao Agent")}</div></button>`;
}
...
async function renderConnections() {
  ...
  main.innerHTML = pageHead("Connections & Credentials", "User-owned OAuth, SecretRef và Unified MCP Hub. Raw credential không có GET API và token Agent chỉ hiển thị một lần.") + `
    ...
    <section class="grid connections space-top">
