...
def run_genos(*args: str, timeout: int = 180) -> dict[str, object]:
    completed = subprocess.run(
        [sys.executable, "-m", "genos", *args, "--json"],
        capture_output=True,
        text=True,
        check=False,
        timeout=timeout,
        env={
            ...
            "PYTHONPATH": "/opt/genos/current/src",
            "HOME": "/var/lib/genos",
            "LANG": "C.UTF-8",
            "LC_ALL": "C.UTF-8",
            "NO_COLOR": "1",
    ...
    try:
        ...
        raise AssertionError(
            f"genos command returned non-JSON output rc={completed.returncode} stderr={completed.stderr[-500:]!r}"
        ) from exc
    if completed.returncode != 0:
        raise AssertionError(f"genos command failed rc={completed.returncode} payload={payload}")
    return payload
...
def bootstrap_with_stdin_secret() -> str:
    ...
    status, payload = request(
        ...
        token=token,
        payload={
            "name": "mvp04-gemini-api-key",
            "provider": "gemini",
            "secret": raw_api_key,
            "consumer_scopes": ["agy-gen"],
        },
    )
    assert status == 201, (status, payload)
    ...
    assert credential["consumer_scopes"] == ["agy-gen"]
    ...
    assert activation["model"] == "gemini-3.7-flash", activation
    assert activation["thinking_level"] == "HIGH", activation
    assert activation["approval_mode"] == "yolo", activation
    assert activation["credential_ref"] == secret_id, activation

    provider_text = (AGENT_ROOT / "provider.json").read_text(encoding="utf-8")
    assert raw_api_key not in provider_text
    assert "GEMINI_API_KEY" not in provider_text
    assert secret_id in provider_text

    # Seed one revision so restart/reboot can prove memory authority survives.
    ...
    (AGENT_ROOT / "acceptance-identity-id.txt").write_text(str(identity["agent_id"]) + "\n", encoding="utf-8")
    print(secret_id)
    return secret_id
...
def execute_real_task(label: str) -> None:
    ...
    result_path = AGENT_ROOT / "tasks" / "results" / f"{task_id}.json"
    ...
    assert marker in serialized, "real model result did not contain the requested marker"
...
def verify_runtime_state() -> None:
    ...
    assert status["provider"]["model"] == "gemini-3.7-flash", status
...
def main() -> int:
    if os.geteuid() == 0:
        raise SystemExit("run MVP-04 acceptance helper as the unprivileged genos identity")
    mode = sys.argv[1] if len(sys.argv) > 1 else "bootstrap"
    if mode == "bootstrap":
        bootstrap_with_stdin_secret()
        return 0
    if mode == "task-before-reboot":
        verify_runtime_state()
        execute_real_task("BEFORE_REBOOT")
        return 0
    if mode == "after-reboot":
        verify_runtime_state()
        execute_real_task("AFTER_REBOOT")
        return 0
