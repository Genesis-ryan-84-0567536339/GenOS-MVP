...
WORK_DIR="${RUNNER_TEMP:-/tmp}/genos-mvp03-fresh-host"
SSH_PORT="${GENOS_MVP03_E2E_SSH_PORT:-2223}"
SSH_USER="genos-ci"
...
EVIDENCE="$WORK_DIR/mvp03-fresh-host-evidence.json"
...
echo "==> Verify pinned Ubuntu 24.04 fresh-host image"
...
instance-id: genos-mvp03-${TESTED_SHA}
local-hostname: genos-mvp03-e2e
EOF
...
echo "==> Owner auth + credential lifecycle + negative leak checks"
ssh_guest "sudo -u genos env PYTHONPATH=/opt/genos/current/src python3 /opt/genos/current/ci/mvp03-auth-e2e.py initial"

echo "==> Idempotent installer rerun"
ssh_guest "$INSTALL_CMD" > "$WORK_DIR/rerun.json"
...
echo "==> Reboot persistence/recovery"
...
verify_core
ssh_guest "sudo -u genos env PYTHONPATH=/opt/genos/current/src python3 /opt/genos/current/ci/mvp03-auth-e2e.py verify"
...
payload = {
    ...
    "second_owner_rejection": "PASS",
    "session_hash_only": "PASS",
    "credential_add_rotate_test_disable": "PASS",
    "consumer_scopes": "PASS",
    "secret_provider_permissions": "PASS",
...
echo "GENOS_MVP03_FRESH_HOST_E2E_PASS"
